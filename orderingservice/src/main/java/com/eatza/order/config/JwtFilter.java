 ...
 * 
 * final HttpServletRequest request = (HttpServletRequest) req; final
 * HttpServletResponse response = (HttpServletResponse) res; final String
 ...
 * InvalidTokenException("Missing or invalid Authorization header"); }
 * 
 * final String token = authHeader.substring(7);
 * 
 * try { final Claims claims =
 * Jwts.parser().setSigningKey("secretkey").parseClaimsJws(token).getBody();
 * 
 * 
 * request.setAttribute("claims", claims); } catch (Exception e) { throw new
 * InvalidTokenException(e.getMessage());
 * 
 * }
 * 
